...
import { Timeline as Vis } from "vis";
...
export default class Timeline extends Component {
  ...
  onMove(item) {
    ...
    try {
      if (this.isImage(item)) {
        ...
        duration = timeManager.subDuration(end, start);
        resource = this.getResourceId(item);
      }
    } catch (error) {}
  ...
  itemMove(item) {
    if (item.start.getUTCFullYear() < 1970) return null;
    // Deny move before zero time
      ...
      if (collision.length === 0) {
        ...
        if (
          ...
          if (item.start === null) {
            return null;
          } // Not enough space at begining of timeline
        } else {
          // Put after
          item.className =
            item.className === "video"
  ...
  isImage(item) {
    ...
    const mime = this.props.resources[resId].mime;
    const isImage = new RegExp(/^image\//).test(mime);
    return isImage;
  }
  getResourceId(item) {
    const itemIndex = Number(item.id.split(":")[1]);
    const track = TimelineModel.findTrack(this.props.items, item.group);
    return track.items[itemIndex].resource;
  }
  hasNeighbours(item) {
    const tracks = item.group.includes("video")
      ? this.props.items.video
      : this.props.items.audio;
    for (let i = 0; i < tracks.length; i++) {
      if (tracks[i].id == item.group) {
        const currentTrack = item.id.split(":")[0];
        return tracks[i].items.length > 0 && currentTrack != item.group;
      }
    }
  }
  shouldMove(item) {
    const currentTrack = item.id.split(":")[0];
    return currentTrack === item.group;
  }
}
