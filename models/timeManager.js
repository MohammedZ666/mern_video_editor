...
export default {
  ...
  toSeconds(time) {
    ...
    const seconds = Number(timeArray[0]);
    const millis = Number(timeArray[1]);
    const totalSeconds = hours * 3600 + mins * 60 + seconds + millis / 1000;
    return totalSeconds;
  },

  /**
   * Returns the diffrence between to time string
   *
   * @param {string} t1 in string
   * @param {string} t2 in string
   * @return {number} t1-t2
   */
